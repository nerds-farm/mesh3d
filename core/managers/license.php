...
namespace Mesh3d\Core\Managers;

use Mesh3d\Base\Base_Global;
...
class License {
    ...
    public function __construct() {
        //$addons = \Mesh3d\Plugin::instance()->get_addons();
        //add_action('elementor/mesh3d/init_license', [$this, 'init_license']);
        add_action('elementor/mesh3d/dash/action', [$this, 'execute_action']);
        add_action('elementor/mesh3d/dash/addon_license', [$this, 'dash_addon_license']);
    }
    ...
    public function init_license($plugin) {
        if (is_admin()) {
            if (!$plugin->is_free()) {
                ...
                $this->updaters = new \Mesh3d\Modules\Update\Edd\Edd($addon);
            }
        }
    }

    public function execute_action($action) {
        $licenses_manager = \Mesh3d\Plugin::instance()->licenses_manager;
