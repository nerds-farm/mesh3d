...
class e_threed_class {
    ...
    constructor(e) {
        ...
        //
        //mouse (questi parametri sono da uniformare meglio)
        this.mouseX = 0;
        this.mouseY = 0;

        this.mouseOffset = new THREE.Vector2();
        
        this.mouseDB = new THREE.Vector2();
        this.raycasterDB = new THREE.Raycaster();
    ...
    updateData3d_hotpoints($id, $settings){
        ...
        this.markerMarkerShapeCustom = $settings.marker_sahpe_custom || 'M140.8,140.8H0V0h140.8V140.8z';
        if(this.markerShapeStyle == 'custom'){
            this.markerShapeStyle = this.markerMarkerShapeCustom;
        }
    ...
    updateData3d(){
        ...
        // target apply ...
        this.cameraTarget.position.set(this.cameraTargetX,this.cameraTargetY,this.cameraTargetZ);
    ...
    add_triggerDbClick(){
        ...
        /*
        // --------------------------------------
        //per gestire l'intersect sugli objects nella pagina e registrare l'emento in cui mi trovo
        ...
        function onDblClick(event){
            this.mouseDB.x = ( event.clientX / w ) * 2 - 1;
            this.mouseDB.y = - ( event.clientY / h ) * 2 + 1;
        
            this.raycasterDB.setFromCamera(mouseDB, camera);
            
            let intersects = this.raycasterDB.intersectObject(box);
    ...
    writeWidgetPanel($id){
        ...
        if(elementorFrontend.isEditMode() && this.punti[$id] && this.punti[$id].hotpointsList.length){
            // points
            jQuery('.elementor-element-'+$id+'.elementor-widget-e-3d-points .e-threed-widget-info svg path').attr('d',this.markerShapeStyle);
            // triggers
            let trigs = '';
            this.punti[$id].hpTrigggers.forEach(function(el,i){
                trigs += el+' ';
            });
            jQuery('.elementor-element-'+$id+'.elementor-widget-e-3d-points .e-threed-widget-info .e-threed-widget-pointstriggers').text(trigs);
        }
    }
    /************************ METHODS *********************** */
    ...
    generateRaycaster(){
        ...
        let onDocumentMouseMove = (event) => {
                ...
                //this.mouseOffset = {x: ( (event.offsetX) - this.windowHalfX ), y: ( event.offsetY - this.windowHalfY )}
                ...
                this.raycasterDB.setFromCamera( this.pointer, this.camera );
                this.oids.forEach((o,i)=>{
                    
                    if(this.oggetti[o] && this.oggetti[o].type == 'object'){
                        //console.log(this.oggetti[o])
                        ...
                        //console.log(this.raycaster.ray);
                        //console.log(this.oggetti[o].object3D)
                        let box = this.oggetti[o].object3d;
                        let intersects = this.raycasterDB.intersectObject(box);
                        ...
                        if (intersects.length > 0){
                            ...
                            this.intersectsPoint.x = o.point.x;
                            this.intersectsPoint.y = o.point.y;
                            this.intersectsPoint.z = o.point.z;
                            
                        }else{
                            
                            this.intersectsPoint.x = 0;
                            this.intersectsPoint.y = 0;
                            this.intersectsPoint.z = 0;
                        }
                ...
                */
                ...
                // -------------------------------------
                //per la gestione l'intersect sui punti e mostrare il tooltip
                this.pids.forEach((el,i) => {
                    this.punti[el].hotpointsList.forEach((element, index) => { 
                        
                        let tooltip = jQuery('.elementor-element.elementor-element-'+el+'.elementor-widget-e-3d-points .e-3d-tooltip').eq(index);           
                        ...
                        if ( this.intersects.length > 0 ) {
                            ...
                            //console.log(this.isPointIntersect);
                        }else{
                            this.isPointIntersect = false;
    ...
    positionMesh($id){
        ...
        if(this.oggetti[$id].object3d) this.oggetti[$id].object3d.rotation.x = THREE.MathUtils.degToRad (this.oggetti[$id].settings.geometryMeshRotX);
        if(this.oggetti[$id].object3d) this.oggetti[$id].object3d.rotation.y = THREE.MathUtils.degToRad (this.oggetti[$id].settings.geometryMeshRotY);
        if(this.oggetti[$id].object3d) this.oggetti[$id].object3d.rotation.z = THREE.MathUtils.degToRad (this.oggetti[$id].settings.geometryMeshRotZ);
    ...
    */
    ...
    calcSpriteSize(){
        ...
        this.pids.forEach((element, index) => { 
            ...
            this.punti[element].hotpointsList.forEach((el, i) => { 
                ...
                */
                this.scaleMarker = this.punti[element].marker.size/5000; // 0.0001;
                var distance = this.camera.position.distanceTo( el.elem.position );
                let scale = this.scaleMarker / this.camera.zoom;
                scale = scale * (distance);
               
                
                el.elem.scale.set(scale,scale,scale);
                //el.elem.scale.set(this.scaleMarker,this.scaleMarker,this.scaleMarker);
    ...
    loadMarker($id, el,$cb = null){
        ...
        //
        // Svg marker
        const svgMarkup = '<svg xmlns="http://www.w3.org/2000/svg"><path d="'+this.markerShapeStyle+'"></path></svg>';
        ...
        svgData.paths.forEach((path, i) => {
            ...
            shapes.forEach((shape, j) => {
                ...
                //let sp = new THREE.Sprite( new THREE.SpriteMaterial({color: 0xCCCCCC,sizeAttenuation: false, transparent: true}))
                ...
                //const points = new THREE.Points( sgeometry, material );
                ...
                smesh.rotation.x = THREE.MathUtils.degToRad (-180);
   ...
   updateHelperMesh($id){
    ...
    updateGeometryMesh(){
        ...
        this.writeWidgetPanel(this.activeId);
        this.render();
    }
    updateControls(){
    ...
    elementChange($id, propertyName, settings, isMultiple) {
        ...
        if ('geometry_mesh_rotx' === propertyName) {
            this.oggetti[$id].settings.geometryMeshRotX = settings.geometry_mesh_rotx ? settings.geometry_mesh_rotx.size : 0;
            this.oggetti[$id].object3d.rotation.x = THREE.MathUtils.degToRad (this.oggetti[$id].settings.geometryMeshRotX);
            this.writeWidgetPanel($id);
            this.render();
        }
        if ('geometry_mesh_roty' === propertyName) {
            this.oggetti[$id].settings.geometryMeshRotY = settings.geometry_mesh_roty ? settings.geometry_mesh_roty.size : 0;
            this.oggetti[$id].object3d.rotation.y = THREE.MathUtils.degToRad (this.oggetti[$id].settings.geometryMeshRotY);
            this.writeWidgetPanel($id);
            this.render();
        }
        if ('geometry_mesh_rotz' === propertyName) {
            this.oggetti[$id].settings.geometryMeshRotZ = settings.geometry_mesh_rotz ? settings.geometry_mesh_rotz.size : 0;
            this.oggetti[$id].object3d.rotation.z = THREE.MathUtils.degToRad (this.oggetti[$id].settings.geometryMeshRotZ);
            this.writeWidgetPanel($id);
            this.render();
        }
        ...
        if ('shadow_type' === propertyName) {
            this.shadowType = settings.shadow_type || 'PCFSoftShadowMap';
        ...
        if ('marker_sahpe_custom' === propertyName) {
            this.markerShapeStyle = settings.marker_sahpe_custom || 'M140.8,140.8H0V0h140.8V140.8z';
